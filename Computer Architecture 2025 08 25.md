...
**Definition.** If a fraction `f` of execution time is improved by a factor `s`, then:

```
T_new = T_old * ((1 - f) + f/s)
Speedup = T_old / T_new = 1 / ((1 - f) + f/s)
```

**Example.** If 10% of time (`f = 0.10`) runs twice as fast (`s = 2`):

```
T_new = T_old * (0.9 + 0.1/2) = 0.95 * T_old
Speedup = 1 / 0.95 ≈ 1.0526
```
...
```
T_CPU = Instruction_Count (IC) * CPI * t_clk
T_CPU = (IC * CPI) / Clock_Rate,  with  t_clk = 1 / Clock_Rate
```

Splitting into CPI and clock period is useful because different design choices affect them differently.
...
* **Instruction Set Architecture (ISA):** Richer instructions can reduce IC. Example: AES instructions (Intel AES-NI, ARM Cryptography Extensions) replace long software sequences with a few instructions.
  ...
  * **Caveat:** Fewer instructions do not guarantee faster runtime. CPI and clock period also change.
...
```
CPI_avg = sum_i (fraction_of_class_i * CPI_i)
```
...
* **Pipelining:** Overlaps instructions across stages (Fetch, Decode, Execute, Memory, Writeback). Reduces effective CPI toward ~1 for simple in-order designs when hazards are minimal.
* **Caches:** Cut memory access latency. Typical latencies: L1 a few cycles; main memory on the order of 100 cycles. Caches reduce stall cycles, lowering CPI for loads/stores.
...
# 6) What controls clock cycle time (t_clk) and clock rate

* **Process technology:** Smaller, faster transistors support a shorter clock period (higher frequency). Say “shorter clock period,” not “smaller clock.”
...
* **ISA (indirectly):** Very complex decoding can lengthen the decode stage unless further subdivided, potentially lengthening `t_clk`.

**Historical note.** The “megahertz myth” era: Intel NetBurst (Pentium 4) deepened pipelines (~20–31 stages) to raise frequency, but IPC dropped due to higher misprediction penalties and other effects. Higher GHz did not guarantee higher performance because:

```
Performance ∝ Clock_Rate / (IC * CPI)
```
...
* **ISA:** The contract visible to compilers/assembly (registers, instructions, semantics). Examples: x86, ARM, MIPS, RISC-V.
* **Microarchitecture:** How the ISA is realized (pipeline depth, caches, predictors, OoO, execution units).
* **Implementation circuits:** Gate-level choices (e.g., ripple-carry vs carry-lookahead adder). These influence stage delays and power; they are below the ISA and support the chosen microarchitecture.
...
* **MIPS32 classic:** No real `blt` instruction. Assemblers offer `blt` as a pseudo-instruction that expands to:
  ...
  * `bne r3, $zero, L1`
* **RISC-V:** Real compare-and-branch opcodes such as `blt rs1, rs2, L1`. No extra `slt` needed for simple relational branches.

“risk file / disk file” refers to **RISC-V**, an open ISA. Using RISC-V does not require paying ISA licensing fees. Purchasing third-party microarchitecture IP is a separate business choice.
...
* **CPU A (MIPS-style):**
  ...
  * CPI: branch instructions take 2 cycles; all other instructions take 1 cycle. The `slt` counts as “other.”
  * Dynamic mix on A: 20% branch, 20% `slt` compares, 60% other.
* **CPU B (RISC-V-style):**
...
We compare execution time: `T = IC * CPI * t_clk`.

## 9.1 Instruction Count

Let `IC_A` be A’s dynamic instruction count. On A, 20% are branches and 20% are the matching `slt`s. On B we fuse and remove those `slt`s:

```
IC_B = IC_A - 0.20 * IC_A = 0.80 * IC_A
```
...
* **CPU A:**
  ...
  ```
  CPI_A = 0.20*2 + 0.20*1 + 0.60*1 = 1.20
  ```
* **CPU B:**
  The fused branch count equals A’s branch count, i.e., `0.20 * IC_A`. As a fraction of B’s stream:

  ```
  branch_fraction_B = (0.20 * IC_A) / IC_B = 0.20 / 0.80 = 0.25
  CPI_B = 0.25*2 + 0.75*1 = 1.25
  ```

## 9.3 Clock period ratio

Given `f_A = 1.25 * f_B`, the periods relate as `t_A = 0.8 * t_B`.

## 9.4 Execution time ratio and relative performance

```
T_A / T_B = (IC_A / IC_B) * (CPI_A / CPI_B) * (t_A / t_B)
          = (1 / 0.80) * (1.20 / 1.25) * 0.80
          = 0.96
```
...
```
Perf_A / Perf_B = T_B / T_A ≈ 1 / 0.96 ≈ 1.0417
```
...
* **Weighted CPI:** `CPI_avg = sum_i (p_i * CPI_i)`, with `p_i` the dynamic fraction of class `i`.
* **Frequency vs pipeline:** Deeper pipelines can reduce `t_clk` but often increase CPI via larger hazard penalties.
...
* **Clock rate:** `f_A = 1.25 * f_B` → `t_A = 0.8 * t_B`.

**Instruction count.** Let `IC_A` be A’s count. B removes the separate `slt`s: `IC_B = 0.8 * IC_A`.

**Average CPI.**

* A: `CPI_A = 0.20*2 + 0.20*1 + 0.60*1 = 1.20`.
* B: branch fraction becomes `0.20 / 0.80 = 0.25`. `CPI_B = 0.25*2 + 0.75*1 = 1.25`.
...
A is ~4% faster (`Perf_A / Perf_B ≈ 1 / 0.96 ≈ 1.042`).
...
```
CPI_avg = sum_i (p_i * CPI_i),  where  sum_i p_i = 1
```
...
* Reasons registers exist even with caches: lower latency, higher multiported bandwidth to the ALUs, compact encodings, lower energy per access, precise compiler-visible state.

**Instruction classes.**

* **Compute:** integer/fp add, sub, mul, logic, shifts, vector ops.
* **Data movement:** register-to-register moves, and load/store between memory and registers. “Load” reads from memory into a register. “Store” writes a register value to memory.
...
* **Compiler usability:** Expose primitives compilers can recognize and exploit regularly.
* **Code size vs performance:** Small encodings help instruction-cache footprint. Important for embedded.
...
* **Instruction length** is separate.
  ...
  * Do not equate “32-bit ISA” with “all instructions are 32 bits long.”

# 6) RISC vs CISC: historical drivers and today’s reality

* **Classic CISC:** memory-operand arithmetic to save code bytes when registers were few and memory was not much slower than the CPU.
* **Classic RISC:** load/store discipline. Move to registers, compute in registers, store results. Simplifies pipelines, improves frequency, and eases scheduling.
* **Modern cores:** x86 decoders translate complex instructions into simpler micro-ops scheduled and executed RISC-style. Architecturally CISC, microarchitecturally RISC-like.
...
* “SIS call / SIS return” → **system call** entry/return. Examples: x86 `syscall/sysret`, RISC-V `ecall/sret`, MIPS `syscall/eret`.
...
```
T_CPU = IC * CPI * t_clk
Perf ∝ Clock_Rate / (IC * CPI)
```

When comparing designs, analyze all three terms. Cutting IC can raise CPI or lengthen `t_clk`. Faster clocks can raise CPI (deeper pipelines, higher branch penalties). Weighted CPI must reflect the new instruction mix after any ISA change.
